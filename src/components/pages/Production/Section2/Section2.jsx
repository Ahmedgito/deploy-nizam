...
const PrevArrow = ({ onClick }) => {
  return (
    <div
      className="absolute top-1/2 left-[-40px] transform -translate-y-1/2 bg-[#1c2c39] hover:bg-orange-500 rounded-full cursor-pointer z-10 w-10 h-10 flex items-center justify-center shadow-lg"
      onClick={onClick}
    >
...
const Section2 = React.forwardRef((props, ref) => {
  ...
  const settings = {
    ...
    nextArrow: <NextArrow />,
    prevArrow: <PrevArrow />,
    beforeChange: (current, next) => setActiveSlide(next), // Track the active slide index
    responsive: [
      {
        breakpoint: 768,
        settings: {
  ...
  // Function to add 'active' class to the headings based on the active slide
  const getActiveHeadingClass = (headingIndex) => {
    if (headingIndex === 0 && activeSlide <= 1) return "text-orange-500 font-bold";
    if (headingIndex === 1 && activeSlide >= 2 && activeSlide <= 3) return "text-orange-500 font-bold";
    if (headingIndex === 2 && activeSlide >= 4) return "text-orange-500 font-bold";
    return "text-[#28303D] font-semibold";
  };  
    ...
    <section className="relative mt-20 -mb-28 bg-white">
      <div className="max-w-full mx-auto px-4 md:px-0">
        <div className="font-montserrat md:text-left text-center md:w-5/6 mx-auto">
          ...
          <p className="text-center text-lg md:text-lg font-bold mt-8 mb-6">
            <span className={`md:inline hidden ms-20 ${getActiveHeadingClass(0)}`}>PRODUCT STEWARDSHIP</span>
            <span className={`md:inline hidden ms-20 me-20 ${getActiveHeadingClass(1)}`}>SOCIAL STEWARDSHIP</span>
            <span className={`md:inline hidden ${getActiveHeadingClass(2)}`}>ENVIRONMENTAL STEWARDSHIP</span>
          </p>
        </div>
      </div>
